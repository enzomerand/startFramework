...
class Auth{
	...
	protected function createSession($user_id, $cookie = false){
		...
		if($cookie === true){
			setcookie(self::COOKIE_NAME, serialize([$user_id, $selector, $token[0], $data]), $expire, '/', DOMAIN, $this->isSecure(), 1);
	...
	private function updateSession($selector, $cookie = false){
		...
		if($cookie === true)
			setcookie(self::COOKIE_NAME, serialize([user_id, $selector, $token[0], $data]), strtotime($get_session->users_logged_expires), '/', DOMAIN, $this->isSecure(), 1);
		else{
			$_SESSION[self::COOKIE_NAME] = serialize([user_id, $selector, $token[0], $data]);
	...
	public function login($user_email, $user_password, $remember_me){
		if($user_email != NULL && $user_password != NULL){
			...
			if($user){
				...
				elseif($user->user_account_activate != 1 && $this->active_account === true)
							...
							if(isset($repeat_password) && $repeat_password == $user_password) {
								if(strlen($user_password) >= 8){
									...
									if(isset($captcha)) {
										...
										if($resp->isSuccess()){
											$my_folder = explode('@', $user_email)[0];
											if (!file_exists(ROOT . '/public/assets/files/users/' . $my_folder)){
												if(!mkdir(ROOT . '/public/assets/files/users/' . $my_folder . '/view/', 0777, true) || !mkdir(ROOT . '/public/assets/files/users/' . $my_folder . '/no-view/', 0777, true))
													$error = 'Erreur lors de la création de votre répertoire personnel. Contactez le support.';
												else {
												...
												$error = 'Erreur, un répertoire personnel ayant le même nom que le votre est déjà existant. Contactez le support.';
										}else
